...
if [ -d /etc/httpd/conf.d ]; then
  ...
  CONF=/etc/apache2/sites-enabled/$1.conf
fi
HOSTNAME="$(hostname -f)"
if [ -z $1 ]; then
...
echo "          </Directory>" >> $CONF
echo "          ErrorLog \"|/usr/bin/cronolog /var/www/$1/logs/%Y/%m/%Y%m%d-error.log\"" >> $CONF
echo "          CustomLog \"|/usr/bin/cronolog /var/www/$1/logs/%Y/%m/%Y%m%d-access.log\" combined" >> $CONF
...
echo "$COMMENT          ErrorLog \"|/usr/bin/cronolog /var/www/$1/logs/%Y/%m/%Y%m%d-ssl-error.log\"" >> $CONF
echo "$COMMENT          CustomLog \"|/usr/bin/cronolog /var/www/$1/logs/%Y/%m/%Y%m%d-ssl-access.log\" combined" >> $CONF
...
echo "$COMMENT          SSLProtocol All -SSLv2 -SSLv3" >> $CONF
